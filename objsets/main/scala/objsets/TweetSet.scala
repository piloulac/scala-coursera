...
abstract class TweetSet {
  ...
  /**
    ...
    *
    */
  def filter(p: Tweet => Boolean): TweetSet = filterAccumulator(p, new Empty)

  /**
    * This is a helper method for `filter` that propagetes the accumulated tweets.
    */
  def filterAccumulator(p: Tweet => Boolean, acc: TweetSet): TweetSet

  /**
    * Returns a new `TweetSet` that is the union of `TweetSet`s `this` and `that`.
    *
    */
  def union(that: TweetSet): TweetSet = filterAccumulator(_ => true, that)
...
class Empty extends TweetSet {
  ...
  def filterAccumulator(p: Tweet => Boolean, acc: TweetSet): TweetSet = acc
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAccumulator(p: Tweet => Boolean, acc: TweetSet): TweetSet =
    if (p(elem))
      left.filterAccumulator(p, right.filterAccumulator(p, acc.incl(elem)))
    else
      left.filterAccumulator(p, right.filterAccumulator(p, acc))
