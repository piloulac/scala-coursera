/**
  * Created by pierrelouislacorte on 15/10/2017.
  */
...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    ...
    for (char <- chars) {
      if (A contains char) {
        val count = A.getOrElse(char, 0) + 1
        A += (char -> count)
      } else
        A += (char -> 1)
    }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] =
    freqs.sortWith(_._2 < _._2).map(e => Leaf(e._1, e._2))
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case left :: right :: tail => (makeCodeTree(left, right) :: tail).sortWith((n1, n2) => weight(n1) < weight(n2))
    case _ => trees
  }
  ...
  def until(isSingleton: List[CodeTree] => Boolean, combine: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
    ...
    else until(isSingleton, combine)(trees)
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decodeAcc(remaining: CodeTree, bits: List[Bit]): List[Char] = remaining match {
      case Leaf(char, _) => if (bits.isEmpty) List(char) else char :: decodeAcc(tree, bits)
      case Fork(left,right,_,_) => if (bits.head == 1) decodeAcc(right,bits.tail) else decodeAcc(left,bits.tail)
    }
