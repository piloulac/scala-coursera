...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (block, move) #:: tail => {
      val newNeighborsPossible = newNeighborsOnly(neighborsWithHistory(block, move), explored + block)
      newNeighborsPossible ++ from(tail ++ newNeighborsPossible, explored + block)
    }
  }
  /**
  ...
  lazy val solution: List[Move] = pathsToGoal match {
    ...
    case (_, _) #:: _ => pathsToGoal.minBy(block => block._2.size)._2
  }
